...
async function processPages (html, opts, savedApps, mappings) {
  ...
  const processedApps = appList.extract(mappings.apps, mappings.appsAlt,mappings.batchProcess, html);
  const apps = opts.fullDetail
    ? await processFullDetailApps(processedApps, opts)
    : processedApps;
  let token = R.path(mappings.token, html);
  if (token === undefined && 'tokenAlt' in mappings)
    token = R.path(mappings.tokenAlt,html);
